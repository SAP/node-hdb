...
describe('db', function () {
  ...
  */
  ...
  /*
    Tests the swapping of data types with the TYPE_SWAP_PROC procedure
    - values: Parameters to swap
    - expected: Expected parameters output
    - dataTypeCodes: Expected data type code(s) of the parameter data types
    - done: Done callback to end the test
  */
  function testDataTypeSwapProc(values, expected, dataTypeCodes, done) {
    var statement;
    function prepare(cb) {
      client.prepare('CALL TYPE_SWAP_PROC (?, ?)', function (err, ps) {
        if (err) done(err);
        statement = ps;
        cb(err);
      });
    ...
    function validate(cb) {
      ...
      metadata[0].should.have.property('dataType').which.is.oneOf(dataTypeCodes);
      metadata[1].should.have.property('dataType').which.is.oneOf(dataTypeCodes);

      statement.exec(values, function (err, params) {
        if (err) done(err);
        params.should.eql(expected);
        cb(err);
      });
    }
    ...
    async.waterfall([prepare, validate, dropStatement], done);
  }
  ...
  describeRemoteDB('REAL_VECTOR, HALF_VECTOR (only tested on HANA cloud)', function () {
    var skipTests = false;
    before(function (done) {
      var version = db.getHANAFullVersion();
    ...
    function setUpTable(tableName, columns, dataFormatRequired) {
      // Returns a closure which has access to the parameters above and the 'this'
      // argument will be the describe context when used in a mocha before hook
      return function setUpTableClosure(done, cb) {
        if (skipTests || db.getDataFormatVersion2() < dataFormatRequired) {
          this.skip();
          done();
        } else {
          setCurTableCols(columns);
          db.createTable.bind(db)(tableName, ['ID INT GENERATED BY DEFAULT AS IDENTITY'].concat(columns), null, cb ? cb : done);
        }
      }
    }

    function dropTable(tableName, dataFormatRequired) {
      return function dropTableClosure(done, cb) {
        if (skipTests || db.getDataFormatVersion2() < dataFormatRequired) {
          done();
        } else {
          db.dropTable.bind(db)(tableName, cb ? cb : done);
        }
      }
    }

    function changeSettingReconnect(settings, dataFormatRequired) {
      return function changeSettingReconnectClosure(done, cb) {
        if (skipTests || db.getDataFormatVersion2() < dataFormatRequired) {
          done();
        } else {
    ...
    describe('REAL_VECTOR', function () {
      describe('dynamic length', function () {
        before(function (done) {
          setUpTable('REAL_VECTOR_TABLE', ['A REAL_VECTOR'], 9).apply(this, [done, function (err) {
            if (err) done(err);
            db.createSwapTypeProc('REAL_VECTOR', done);
          }]);
        });
        after(function (done) {
          dropTable('REAL_VECTOR_TABLE', 9).apply(this, [done, function (err) {
            if (err) done(err);
            db.dropSwapTypeProc(done);
          }]);
        });
        ...
        it('should call a procedure with real vector params', function (done) {
          var insertValues = [[1, 2, 3], [4, 5, 6]];
          var expected = {A: Buffer.from("03000000000080400000A0400000C040", "hex"),
            B: Buffer.from("030000000000803F0000004000004040", "hex")};
          testDataTypeSwapProc(insertValues, expected, [96], done);
        });
      });

      describe('vectorOutputType Array', function () {
        before(function (done) {
          changeSettingReconnect({ vectorOutputType: 'Array', packetSizeLimit: Math.pow(2, 19) }, 9)(done, function (err) {
            if (err) done(err);
            db.createSwapTypeProc('REAL_VECTOR', done);
          });
        });
        after(function (done) {
          changeSettingReconnect({ vectorOutputType: 'Buffer', packetSizeLimit: DEFAULT_PACKET_SIZE }, 9)(done, function (err) {
            if (err) done(err);
            db.dropSwapTypeProc(done);
          });
        });
    ...
    describe('HALF_VECTOR', function () {
      describe('dynamic length', function () {
        before(function (done) {
          setUpTable('HALF_VECTOR_TABLE', ['A HALF_VECTOR'], 10).apply(this, [done, function (err) {
            if (err) done(err);
            db.createSwapTypeProc('HALF_VECTOR', done);
          }]);
        });
        after(function (done) {
          dropTable('HALF_VECTOR_TABLE', 10).apply(this, [done, function (err) {
            if (err) done(err);
            db.dropSwapTypeProc(done);
          }]);
        });

        it('should return valid half vectors via callback', function (done) {
          var insertValues = [
            [Buffer.from("050000008F773F4747E83B363EC2", "hex")],
            [[0]],
            [[-100, 200, -3000.458]],
            [[5.960464477539063e-8, -5.960464477539063e-8, 0.00006097555160522461, -0.00006103515625, 1, -1, 65504, -65504]],
            [[0.01177, -5.6832284806445, 64438.18, -59.801068263, -425.9631]],
            [null],
          ];
          var expected = [{A: Buffer.from("050000008F773F4747E83B363EC2", "hex")}, {A: Buffer.from("010000000000", "hex")},
            {A: Buffer.from("0300000040D6405ADCE9", "hex")}, {A: Buffer.from("0800000001000180FF030084003C00BCFF7BFFFB", "hex")},
            {A: Buffer.from("050000000722AFC5DE7B7AD3A8DE", "hex")}, {A: null}];
          testDataTypeValid('HALF_VECTOR_TABLE', insertValues, [97], expected, done);
        });
        ...
        it('should raise input type error', function (done) {
          var invalidTestData = [
            {
              value: 5,
              errMessage: 'Cannot set parameter at row: 1. Wrong input for HALF_VECTOR type'
            },
            {
              value: ['3.12'],
              errMessage: 'Cannot set parameter at row: 1. Wrong input for HALF_VECTOR type'
            ...
            {
              value: Buffer.from("010000003F"),
              errMessage: 'Cannot set parameter at row: 1. Invalid length or indicator value for HALF_VECTOR type'
            },
            {
              value: Buffer.from("00000000"),
              errMessage: 'Cannot set parameter at row: 1. Invalid length or indicator value for HALF_VECTOR type'
            ...
            {
              value: Buffer.from("02000000003C00400042"),
              errMessage: 'Cannot set parameter at row: 1. Invalid length or indicator value for HALF_VECTOR type'
            },
            {value: [], errMessage: 'Cannot set parameter at row: 1. Invalid length or indicator value for HALF_VECTOR type'},
            {
              value: [Infinity],
              errMessage: 'Invalid vector format: exception 337056: Found a non-finite vector element at offset 4\n: type_code=12, index=1'
            },
          ];
          async.each(invalidTestData, testDataTypeError.bind(null, 'HALF_VECTOR_TABLE'), done);
        });

        it('should call a procedure with half vector params', function (done) {
          var insertValues = [[1, 2, 3], [4, 5, 6]];
          var expected = {A: Buffer.from("03000000004400450046", "hex"),
            B: Buffer.from("03000000003C00400042", "hex")};
          testDataTypeSwapProc(insertValues, expected, [97], done);
        });
      });

      describe('vectorOutputType Array', function () {
        before(function (done) {
          changeSettingReconnect({ vectorOutputType: 'Array', packetSizeLimit: Math.pow(2, 19) }, 10)(done, function (err) {
            if (err) done(err);
            db.createSwapTypeProc('HALF_VECTOR', done);
          });
        });
        after(function (done) {
          changeSettingReconnect({ vectorOutputType: 'Buffer', packetSizeLimit: DEFAULT_PACKET_SIZE }, 10)(done, function (err) {
            if (err) done(err);
            db.dropSwapTypeProc(done);
          });
        });
        beforeEach(setUpTable('HALF_VECTOR_TABLE', ['A HALF_VECTOR'], 10));
        afterEach(dropTable('HALF_VECTOR_TABLE', 10));

        it('should return valid real vectors as arrays', function (done) {
          var insertValues = [
            [null],
            [[0]],
            [Buffer.from("01000000B757", "hex")],
            [[52871.139642, 4614.4, -340.8256, -28.77, 5.81370, -0.0233050, -0.002193225, 0.0000405891525]],
            [Buffer.from("05000000003C0040004200440045", "hex")],
            // Rounding test, between 32768 and 65520, precision is limited to 32, so round to nearest
            // even multiple of 32
            [[64400, 64400.001, 64399.999999, 64438.18, 64432, 64431.999999]],
          ];
          var expected = [{A: null}, {A: [0]}, {A: [123.4375]}, {A: [52864, 4616, -340.75,
            -28.765625, 5.8125, -0.0233001708984375, -0.002193450927734375, 4.0590763092041016e-05]},
            {A: [1, 2, 3, 4, 5]}, {A: [64384, 64416, 64384, 64448, 64448, 64416]}];
          testDataTypeValid('HALF_VECTOR_TABLE', insertValues, [97], expected, done);
        });

        it('should insert and return maximum size real vectors', function (done) {
          this.timeout(5000);
          // Mod to maintain precision of at least 1
          var buildVector = Array.from(Array(65000).keys()).map(function (x) {
            return x % 2048;
          });
          var insertValues = [buildVector];
          var expected = [{A: buildVector}];
      ...
      describe('fixed length', function () {
        before(setUpTable('HALF_VECTOR_TABLE', ['A HALF_VECTOR(3)'], 10));
        after(dropTable('HALF_VECTOR_TABLE', 10));

        it('should return valid half vectors via callback', function (done) {
          var insertValues = [
            [[0, 0, 0]],
            [Buffer.from("030000004056405AB05C", "hex")],
            [[-39144.50268, -135.3234127, 4.4220961]],
            [[-0.30908203125, 0.0061, 0.0000142]],
            [[5.960464477539063e-8, 0.00006103515625, 65504]],
            [[-5.960464477539063e-8, -0.00006103515625, -65504]],
            [null],
          ];
          var expected = [{A: Buffer.from("03000000000000000000", "hex")}, {A: Buffer.from("030000004056405AB05C", "hex")},
            {A: Buffer.from("03000000C7F83BD86C44", "hex")}, {A: Buffer.from("03000000F2B43F1EEE00", "hex")},
            {A: Buffer.from("0300000001000004FF7B", "hex")}, {A: Buffer.from("0300000001800084FFFB", "hex")}, {A: null}];
          testDataTypeValid('HALF_VECTOR_TABLE', insertValues, [97], expected, done);
        });
        ...
        it('should raise input type error', function (done) {
          var invalidTestData = [
            {
              value: Buffer.from("03000000B057205F2A62004A645D", "hex"),
              errMessage: 'Cannot set parameter at row: 1. Invalid length or indicator value for HALF_VECTOR type'
            },
            {
              value: [3, 4, 5, 6, 7],
              errMessage: 'Cannot set parameter at row: 1. The source dimension is different from the target dimension for HALF_VECTOR type'
            },
            {
              value: [1, 2],
              errMessage: 'Cannot set parameter at row: 1. The source dimension is different from the target dimension for HALF_VECTOR type'
            ...
            {
              value: Buffer.from("010000009E74", "hex"),
              errMessage: 'Cannot set parameter at row: 1. The source dimension is different from the target dimension for HALF_VECTOR type'
            },
          ];
          async.each(invalidTestData, testDataTypeError.bind(null, 'HALF_VECTOR_TABLE'), done);
