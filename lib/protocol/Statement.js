...
Statement.prototype._normalizeInputParameters = function _normalizeInputParameters(
  ...
  function getTypeLength(metadata) {
  ...
  var parameters = {
    ...
    lengths: inputParameterMetadata.map(getTypeLength),
  };

  parameters.values = Array.isArray(values) ? values : inputParameterMetadata.filter(isDefined).map(getObjectValue);

  var vals;
  if (parameters.values.length && Array.isArray(parameters.values[0])
    && (!isVector(parameters.types[0]) || Array.isArray(parameters.values[0][0]) || Buffer.isBuffer(parameters.values[0][0])
        || (parameters.values[0].length > 0 && parameters.values[0][0] == null))) {
