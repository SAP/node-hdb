...
function ResultSet(connection, rsd, options) {
  ...
  this._fetchSize = options.fetchSize || ResultSet.DEFAULT_FETCH_SIZE;
  this._readSize = options.readSize || Lob.DEFAULT_READ_SIZE;
  this._nestTables = options.nestTables || false;
  if (options.rowsAsArray) {
    this._columnNameProperty = false;
  } else {
    this._columnNameProperty = options.columnNameProperty || 'columnDisplayName';
  }
}
...
ResultSet.prototype.createParser = function createParser(options) {
  return Parser.create(this.metadata).createTransform(this, util.extend({
    ...
    nameProperty: this._columnNameProperty
  }, options));
};
