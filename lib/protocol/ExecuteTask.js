...
ExecuteTask.prototype.getParameters = function getParameters(availableSize, cb) {
  ...
  function next() {
    ...
    var rowToBeProcessed;
    if (!self.writer.hasParameters) {
      try {
        ++row;
        rowToBeProcessed = self.parameterValues.shift();
        self.writer.setValues(rowToBeProcessed);
      } catch (err) {
    ...
    if (self.writer.length > remainingSize) {
      if (!self.parameterValues.length && rowToBeProcessed) {
        self.writer.reset();
        self.parameterValues.unshift(rowToBeProcessed);
      }
      if (self.autoCommit) {
        self.needFinializeTransaction = true;
        self.autoCommit = false;
