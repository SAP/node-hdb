...
function ExecuteTask(connection, options, callback) {
  ...
  if (values.length && util.isArray(values[0])) {
    ...
    case FunctionCode.DELETE:
      break;
    default:
      throw new Error('Statement in batch must be DDL or DML');
    }
  } else {
    this.parameterValues = [values];
  }
...
ExecuteTask.prototype.run = function run(next) {
  ...
  function finalize(err) {
    ...
    if (err) {
      return self.sendRollback(done.bind(null, err));
    }
...
ExecuteTask.prototype.pushReply = function pushReply(reply) {
  ...
  if (reply.rowsAffected) {
    if (!util.isArray(this.reply.rowsAffected)) {
      this.reply.rowsAffected = [this.reply.rowsAffected];
    }
    this.reply.rowsAffected = this.reply.rowsAffected.concat(reply.rowsAffected);
  }
};
...
ExecuteTask.prototype.getParameters = function getParameters(availableSize, cb) {
  ...
  function next() {
    ...
    self.writer.getParameters(remainingSize, function push(err, buffer) {
      if (err) {
        return cb(err);
      }
