...
function ExecuteTask(connection, options, callback) {
  ...
  this.writer = new Writer(
    {
      types: options.parameters.types,
      fractions: options.parameters.fractions,
      lengths: options.parameters.lengths
    },
    { useCesu8: connection.useCesu8, spatialTypes: connection.spatialTypes }
  );
  var values = options.parameters.values;
  if (values.length && Array.isArray(values[0])
    && (!isVector(options.parameters.types[0]) || Array.isArray(values[0][0])
        || Buffer.isBuffer(values[0][0]) || (values[0].length > 0 && values[0][0] == null))) {
