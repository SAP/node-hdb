...
const FunctionCode = common.FunctionCode;
const SegmentKind = common.SegmentKind;
const PartKindName = common.PartKindName;
const {TopologyUpdateRecord} = require('../../protocol/ConnectionTopology');

const PACKET_HEADER_LENGTH = common.PACKET_HEADER_LENGTH;
const DEFAULT_SEGMENT_SIZE = common.DEFAULT_PACKET_SIZE - PACKET_HEADER_LENGTH;
...
/** @summary Add received list of topology information to TopologyUpdateRecord list
 *  @param {Array} value An array of topology information, each element is an array
 *  of objects with {name: <topology_info_name>, type: <value_type>, value: <value>}.
 *  I.e.
 *  \[
 ...
 *  \]
 */
Reply.prototype.addTopologyUpdateRecords = function addTopologyUpdateRecords(value) {
  if (!Array.isArray(value)) {
    return;
  }
  value.forEach(updatedTopology => {
    const topologyUpdateRecord = TopologyUpdateRecord.create(updatedTopology);
    if (topologyUpdateRecord) {
      this.topologyUpdateRecords.push(topologyUpdateRecord);
    }
  });
};

Reply.prototype.add = function add(part) {
  const name = util._2cc(PartKindName[part.kind]);
  var value = data[part.kind].read(part);
  if (/^resultSet/.test(name) || name === 'tableName') {
    this.addResultSetFragment(name, value);
  } else if (name === 'topologyInformation') {
    this.addTopologyUpdateRecords(value);
