...
Reader.prototype.readLongDate = function readLongDate() {
  ...
  if (typeof value === 'string') {
    ...
    return value.substring(0, index) + bignum.lpad7(value.substring(index) - 1);
  } else {
    return value - 1;
  }
};
...
Reader.prototype.readLob = function readLob(type) {
  ...
  // offset 0
  var sourceType = this.buffer[this.offset];
  this.offset += 1;
  // offset 1
  ...
  // is last
  var isLast = !! (options & LobOptions.LAST_DATA);
  if (isLast) {
    //return chunk;
  }
  if (!byteLength && !charLength) {
    return null;
  }
  return new LobDescriptor(sourceType || type,
    locatorId, charLength, byteLength, options, chunk);
};
...
function LobDescriptor(type, locatorId, charLength, byteLength, options, chunk) {
  this.type = type;
...
Object.defineProperties(LobDescriptor.prototype, {
  ...
  size: {
    get: function getSize() {
      ...
      if (this.type > LobSourceType.BLOB) {
        return this.chunk.toString('utf-8').length;
      }
