...
Reader.prototype.readLob = function readLob(type) {
  ...
  return this.lobFactory.createLob(new LobDescriptor(type, locatorId,
    options,
    chunk));
};
...
function LobDescriptor(type, locatorId, options, chunk) {
  ...
  this.chunk = chunk;
  if (type === LobSourceType.NCLOB) {
    this.size = chunk.toString('utf-8').length;
  } else if (type === LobSourceType.CLOB) {
    this.size = chunk.toString('ascii').length;
  } else {
