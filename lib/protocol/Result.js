...
Result.prototype.handle = function handle(err, reply, cb) {
  ...
  case FunctionCode.SELECT_FOR_UPDATE:
    this.handleQuery(
      this.createResultSets(reply.resultSets), cb);
    return;
...
Result.prototype.handleDML = function handleDML(rowsAffected, resultSets, cb) {
  var resultSet = resultSets.length ? resultSets[0] : null;
  cb(null, rowsAffected, resultSet);
};

Result.prototype.handleQuery = function handleQuery(resultSets, cb) {
...
Result.prototype.handleDBCall = function handleDBCall(params, resultSets, cb) {
  function done(err, results) {
    if (err) {
      return cb(err);
    }
