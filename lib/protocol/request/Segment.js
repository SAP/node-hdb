...
Segment.prototype.toBuffer = function toBuffer() {
  var length = SEGMENT_HEADER_LENGTH;
  var buffers = [];
  for (var i = 0; i < this.parts.length; i++) {
    var part = partToBuffer(this.parts[i], this.useCesu8);
    length += part.getLength();
    buffers.push(part);
  }

  var header = Buffer.allocUnsafe(SEGMENT_HEADER_LENGTH);
...
Segment.prototype.updateBuffer = function updateBuffer(buffers, remaining) {
  remaining -= buffers[0].length
  for (var i = 1; i < buffers.length; i++) {
    const part = buffers[i]
    remaining -= part.getLength()
    buffers[i] = part.toBuffer(remaining)
  }
  return buffers
}

function partToBuffer(pd, useCesu8) {
