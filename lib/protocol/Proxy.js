...
class ProxyClient {
      ...
      () => {
        (async () => {
          try {
            ...
            if (util.shouldUseTLS(this._options)) {
              this._upgradeToTLS(cb);
            } else {
              return cb();
            }
          } catch (err) {
            return cb(err); //the socket will get destroyed in the callback
          }
        })();
      },
    );
  ...
  _upgradeToTLS(cb) {
    ...
    const { host, port, ...tlsOptions } = this._options;
    tlsOptions.socket = this._socket;
