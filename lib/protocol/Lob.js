...
function Lob(connection, lob) {
  ...
  this.type = lob.type;
  this.id = lob.locatorId;
  this.byteLength = lob.byteLength;
  this.charLength = lob.charLength;
  this.ended = false;
  this.offset = 0;
  this._data = null;
  if (Buffer.isBuffer(lob.chunk) && lob.chunk.length) {
    var size = lob.chunk.length;
    if (this.type === Lob.TYPE_CLOB) {
      size = lob.chunk.toString('utf-8').length;
    }
    this._data = {
      isLast: lob.isLast,
      chunk: lob.chunk,
