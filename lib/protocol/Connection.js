...
function Connection() {
  ...
  if (process.env.HDB_TRACE_FILENAME) {
    this._filename = process.env.HDB_TRACE_FILENAME;
  }
}
...
Connection.prototype.open = function open(options, cb) {
  ...
  function ondata(chunk) {
    readInitializationReply.call(self, cb);
  }
...
function send(message, receive) {
  ...
  debug('send', message);
  if (this._filename) {
    fs.appendFile(this._filename, util.inspect(message, {
      depth: 9
    }) + '\n');
  }
...
function readReply() {
  ...
  if (util.isFunction(cb)) {
    ...
    try {
      ...
      if (this._filename) {
        fs.appendFile(this._filename, util.inspect(segment));
      }
