...
Connection.prototype._addListeners = function _addListeners(socket) {
  ...
  function ondata(chunk) {
    ...
    if (packet.isReady()) {
      ...
      process.stdout.write(`<< ${self._id}:${packetCount}\n`)
...
Connection.prototype.send = function send(message, receive) {
  this._sendBuffer = [{ message, receive }]
  return this._send(receive)

  // Not all request kinds can be included inside a batch request
  const trigger = !this._sendBuffer
  const buffer = this._sendBuffer ??= []

  buffer.push({ message, receive })

  if (trigger) {
    setImmediate(() => {
      this._send((err, reply) => {
        const { receive } = buffer.shift()
        receive(err, reply)
      })
    })
  }
...
Connection.prototype._send = function send(receive) {
  const sendBuffer = this._sendBuffer
  this._sendBuffer = undefined

  const rawBuffer = []

  let bufferLength = PACKET_HEADER_LENGTH
  for (let i = 0; i < sendBuffer.length; i++) {
    const { message } = sendBuffer[i]
    ...
    bufferLength += buffers.byteLength
    rawBuffer[i] = buffers
  }

  var packet = Buffer.allocUnsafe(PACKET_HEADER_LENGTH);
  ...
  process.stdout.write(`++ ${this._id}:${packetCount}\n`)
  ...
  packet.writeUInt16LE(sendBuffer.length, 20);
  // Filler
  packet.fill(0x00, 22, PACKET_HEADER_LENGTH);

  const socketBuffer = [packet]

  let remaining = bufferLength - PACKET_HEADER_LENGTH
  for (let i = 0; i < sendBuffer.length; i++) {
    const { message } = sendBuffer[i]
    const curRawBuffer = rawBuffer[i]
    message.updateBuffer(curRawBuffer, remaining)
    remaining -= curRawBuffer.byteLength
    for (let i = 0; i < curRawBuffer.length; i++) {
      socketBuffer.push(curRawBuffer[i]);
    }
  }

  socketBuffer._id = packetCount
  this._socketWriteQueue ??= []
  this._socketWriteQueue.push(socketBuffer)
...
Connection.prototype._socketWriteNext = function _socketWriteNext() {
  ...
  process.stdout.write(`>> ${this._id}:${drain._id}\n`)
  this._socket.write(Buffer.concat(drain), () => {/*
    this._socketWriteQueue.shift()
    this._socketWriteNext()*/
  })
}
