...
Connection.prototype.open = function open(options, cb) {
  ...
  self._socket = self._connect(options, function connectListener(err, socket){
    if (err) {
      onerror(err);
      return;
    }
    if(socket) {
      self._socket.removeListener('error', onerror); //remove error handler from old socket
      self._socket = socket;
      self._socket.once('error', onerror); //add error handler to new socket
    }
...
Connection.prototype._addListeners = function _addListeners(socket) {
  ...
  function ondata(chunk) {
    ...
    if (packet.isReady()) {
      ...
      if(compressor.isPacketCompressed(packet.header.packetOptions)) {
        if(packet.header.length <= 0
          || packet.header.length >= packet.header.compressionVarpartLength
          || packet.header.length > self.packetSizeLimit) {
              // client or server internal error (should never happen)
              throw new Error(
                `Packet decompression failed: Invalid packet length: ${packet.header.length}, ` +
                `compressionVarpartLength: ${packet.header.compressionVarpartLength}, ` +
                `packetSizeLimit: ${self.packetSizeLimit}`
              );              
          }
        buffer = compressor.decompress(buffer, packet.header.compressionVarpartLength);
      }   
      packet.clear();
...
Connection.prototype.send = function send(message, receive) {
  ...
  let size = this.packetSizeLimit - PACKET_HEADER_LENGTH;
  let buffer = message.toBuffer(size);
  if(buffer.length > size) {
    return receive(new Error('Packet size limit exceeded'));
  }
  let packet = Buffer.alloc(PACKET_HEADER_LENGTH + buffer.length);
  ...
  if (this._socket) {
    if (message.type != common.MessageType.AUTHENTICATE && 
        message.type != common.MessageType.CONNECT &&  
        this._compressionEnabled &&
        packet.length > MIN_COMPRESS_PKT_LEN) {
      const finalPacket = compressor.compress(packet);
      //Write compressed request packet to socket
      this._socket.write(finalPacket);
    } else {
      // Write uncompressed request packet to socket
...
Connection.prototype.connect = function connect(options, cb) {
  ...
  function connReceive(err, reply) {
    ...
    if (Array.isArray(reply.connectOptions)) {
      ...
      if(compressor.isLZ4CompressionNegotiated(self.connectOptions.compressionLevelAndFlags))  {
