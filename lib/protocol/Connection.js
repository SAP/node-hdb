...
// const DEFAULT_DISTRIBUTION_MODE = common.ClientDistributionMode.STATEMENT;
...
Connection.prototype.send = function send(message, receive) {
  ...
  if (this._clientInfo.shouldSend(message.type)) {
    message.add(PartKind.CLIENT_INFO, this._clientInfo.getUpdatedProperties(this.connectOptions.clientInfoNullValueSupported));
  }
...
Connection.prototype.getAvailableSize = function getAvailableSize(forLobs = false) {
  ...
  availableSize -= this._clientInfo.getUpdatedPropertiesSize(this.useCesu8, this.connectOptions.clientInfoNullValueSupported);
...
Connection.prototype.disconnect = function disconnect(cb) {
  ...
  function done(err, reply) {
    self.destroy();
    // ECONNRESET means the server closed the TCP connection — the desired outcome
    cb(err && err.code !== 'ECONNRESET' ? err : null, reply);
  }
...
Connection.prototype._setClientInfo = function _setClientInfo(key, val) {
  if (val == null) {
    this._clientInfo.removeProperty(key);
  } else {
    this._clientInfo.setProperty(key, String(val));
  }
};
