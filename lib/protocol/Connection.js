...
Connection.prototype.open = function open(options, cb) {
  ...
  this._socket = this._createSocket(options, onready);
};

Connection.prototype._createSocket = function _createSocket(options, cb) {
  options = util.extend(options, {
    allowHalfOpen: false
  });
  var network = options.key ? tls : net;
  var socket = network.connect(options, function onconnect() {
    /* jshint validthis:true */
    this.write(initializationRequestBuffer);
  });
...
Connection.prototype.connect = function connect(options, cb) {
  ...
  if (!algorithm) {
    process.nextTick(function noAuthenticationAlgorithmFound() {
      var err = new Error('No authentication algorithm found');
      err.code = 'EHDBNOAUTHALGO';
      cb(err);
    });
    return;
  }
  ...
  function authReceive(err, reply) {
    ...
    self.send(request.connect({
      authentication: algorithm.getFinalFields(reply.authentication),
