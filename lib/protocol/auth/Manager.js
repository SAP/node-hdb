...
function Manager(options) {
  ...
  this._authMethods = [];
  // Array of pairs {type: Authentication method, error: Error raised}
  var authErrors = [];
  ...
  if (options.authenticationX509 || (!options.user && options.password && options.password.toString().startsWith("-----BEGIN "))) {
    try {
      this._authMethods.push(new X509(options));
    } catch (err) {
      authErrors.push({type: 'X.509', error: err});
    }
  }
  if (!this._authMethods.length) {
    throw noAuthMethodFound(authErrors);
  }
}
...
function noAuthMethodFound(authErrors) {
  var errMessage = 'No authentication method found.';
  if (authErrors) {
    for (var i = 0; i < authErrors.length; i++) {
      errMessage += " " + authErrors[i].type + ': ' + authErrors[i].error.message;
    }
  }
  var error = new Error(errMessage);
  error.code = 'EHDBNOAUTHMETH';
  // Add additional information from errors
  if (authErrors) {
    for (var i = 0; i < authErrors.length; i++) {
      error[authErrors[i].type] = authErrors[i].error;
    }
  }
  return error;
}
