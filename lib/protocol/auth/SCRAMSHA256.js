...
function SCRAMSHA256(options, usePBKDF2) {
  ...
  if (usePBKDF2) this.name = 'SCRAMPBKDF2SHA256';
  else this.name = 'SCRAMSHA256';
...
SCRAMSHA256.prototype.initialize = function initialize(buffer, cb) {
  ...
  if (this.usePBKDF2) var iterations = serverChallengeData[2].readUInt32BE();
  else var iterations = 0;
  calculateProofs(
