...
util.inherits(SCRAMSHA256, Algorithm);

function SCRAMSHA256(options) {
  Algorithm.call(this, 'SCRAMSHA256', options.user);

  this.password = options.password;
  if (util.isString(this.password)) {
    this.password = new Buffer(this.password, 'utf8');
  }
  this.clientChallenge = options.clientChallenge || crypto.randomBytes(
    CLIENT_CHALLENGE_SIZE);
}

SCRAMSHA256.prototype.getInitialData = function getInitialData() {
  return this.clientChallenge;
};

SCRAMSHA256.prototype.getFinalData = function getFinalData(fields) {
  var serverChallengeData = Fields.read({
    argumentCount: 1,
    buffer: fields[1]
  });
  var salt = serverChallengeData[0];
  var serverChallenge = serverChallengeData[1];
  return this.getClientProof([salt], serverChallenge);
};

SCRAMSHA256.prototype.getClientProof = function getClientProof(salts,
  serverChallenge) {
  var self = this;
  ...
  salts.forEach(function scrambleSalt(salt) {
    ...
    offset += 1;
    scramble(self.password, salt, self.clientChallenge, serverChallenge)
      .copy(buf, offset);
