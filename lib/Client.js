...
var hanaOpenTelemetry = require('../extension/OpenTelemetry');
...
Client.prototype.connect = function connect(options, cb) {
  ...
  addDynatraceWrapper(this, {host: this._settings.host, port: this._settings.port,
    dynatraceTenant: this._settings.dynatraceTenant,
    openTelemetryTenant: this._settings.openTelemetryTenant}, connectOptions);
...
function normalizeSettings(settings) {
  for (var key in settings) {
    ...
    } else if (key.toUpperCase() === 'OPENTELEMETRY') {
      var {value, isValid} = util.validateAndGetBoolProperty(settings[key]);
      settings['openTelemetry'] = isValid ? value : true;
    } else if (key.toUpperCase() === 'OPENTELEMETRYTENANT') {
      settings['openTelemetryTenant'] = settings[key];
    }
  }
}
...
function addDynatraceWrapper(client, destinationInfo, connectOptions) {
  // Prefer OpenTelemetry over Dynatrace if both are enabled
  if (hanaOpenTelemetry && hanaOpenTelemetry.isOpenTelemetryEnabled()
    && connectOptions.openTelemetry) {
    destinationInfo.tenant = destinationInfo.openTelemetryTenant;
    hanaOpenTelemetry.openTelemetryConnection(client, destinationInfo);
  } else if (hanaDynatrace && hanaDynatrace.isDynatraceEnabled()
    && connectOptions.dynatrace) {
    destinationInfo.tenant = destinationInfo.dynatraceTenant;
