...
// Bytes are limited to 16 since we are using readUInt128LE
function readFIXED(buffer, bytes, offset, frac) {
  var s;
  var tmpBuffer = Buffer.alloc(16);
  for (var i = 0; i < bytes; i++) {
    tmpBuffer[i] = buffer[i + offset];
  }
  // Convert to unsigned
  if (tmpBuffer[bytes - 1] & 0x80) {
    // Convert back to positive, flip bits and add 1
    var extraOne = true;
    for (var i = 0; i < bytes; i++) {
      if (extraOne) {
        if (tmpBuffer[i] !== 0) {
          tmpBuffer[i] = 0xff - tmpBuffer[i] + 1;
          extraOne = false;
        } else {
          tmpBuffer[i] = 0;
        }
      } else {
        tmpBuffer[i] = 0xff - tmpBuffer[i];
      }
    }
    s = -1;
  } else {
  ...
  // Convert to string
  var fullNum = readUInt128(tmpBuffer, 0);
  var fullStr;
  if (typeof fullNum === 'number') {
    fullStr = fullNum.toString();
  } else {
    fullStr = fullNum;
  }
  ...
  if (l <= frac) {
    i = '0';
    f = zeropad.ZEROS[frac - l] + fullStr;
  } else {
    i = fullStr.substring(0, l - frac);
    f = fullStr.substring(l - frac);
  }
  if (s < 0) {
    return '-' + i + '.' + f;
  } else {
